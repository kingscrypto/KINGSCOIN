...
/**
 * Fees smaller than this (in ukgs) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 KGS.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-kgs-address
                    CScript scriptChange;
